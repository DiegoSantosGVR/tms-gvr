...
async function jamefLogin(body) {
  ...
  if (!idToken) {
    ...
    if (bodyStr.includes('mfa') || bodyStr.includes('otp') || bodyStr.includes('código') || bodyStr.includes('codigo')) {
      return { mfaRequired: true, Session: data.session || data.Session || '', _rawLogin: data };
    }
    console.log('[JAMEF LOGIN] Resposta inesperada:', JSON.stringify(data).slice(0, 300));
    throw new Error('JAMEF: ' + (data.message || data.erro || data.error || 'Resposta inesperada. Verifique os logs.'));
  }
...
async function jamefMFA(body) {
  ...
  if (result.status >= 400) {
    ...
    try { data = JSON.parse(result2.body); }
    catch { throw new Error('Código MFA inválido ou expirado.'); }
    if (result2.status >= 400) throw new Error(data.message || data.erro || 'Código MFA inválido.');
  }
  ...
  if (!idToken) throw new Error('MFA OK mas tokens não retornados. Verifique o endpoint.');
...
async function braspressCotacao(campos) {
  // Obtém JSESSIONID — do popup (POPUP_AUTH = login via browser) ou direto
  let jsessionid = bpSession.jsessionid;

  if (campos.jsessionid && campos.jsessionid !== 'POPUP_AUTH') {
    jsessionid = campos.jsessionid;
    bpSession.jsessionid = jsessionid;
  }

  if (!jsessionid || jsessionid === 'POPUP_AUTH') {
    // Tenta obter nova sessão via login automático no portal
    jsessionid = await braspressLogin(
      campos.email || bpSession.email || '',
      campos.senha || bpSession.senha || ''
    ).catch(() => null);
  }

  if (!jsessionid) {
    throw new Error('Sessão BRASPRESS não disponível. Faça login pelo popup e tente novamente.');
  }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/api/jamef/mfa')     return json(res, 200, { ok: true, ...(await jamefMFA(body)) });
    if (pathname === '/api/jamef/cotacao') return json(res, 200, { ok: true, resultado: await jamefCotacao(body) });

    // ── BRASPRESS ──
    if (req.method === 'GET' && pathname === '/api/braspress/session') {
      if (bpSession.jsessionid) {
        return json(res, 200, { jsessionid: bpSession.jsessionid });
      } else {
        return json(res, 200, { jsessionid: null, msg: 'Sem sessão ativa' });
      }
    }
