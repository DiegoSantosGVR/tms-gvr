...
async function jamefCotacao(campos) {
  let { idToken, accessToken } = campos;
  // POPUP_AUTH = usuário logou via popup, usa cookies do portal diretamente
  if (idToken === 'POPUP_AUTH') {
    throw new Error('Token JAMEF via popup não suportado ainda — use cotação via portal diretamente ou aguarde API oficial.');
  }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'GET' && pathname === '/api/jamef/session') {
      return json(res, 200, { idToken: null, msg: 'Use popup para obter tokens' });
    }
