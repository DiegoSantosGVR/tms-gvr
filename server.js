...
async function jamefCotacao(campos) {
  ...
  // Usa metragemCubica já calculada pelo frontend (soma de todas as caixas)
  const metragemCubica = campos.metragemCubica
    ? parseFloat(Number(campos.metragemCubica).toFixed(4))
    : (comp > 0 && larg > 0 && alt > 0
  ...
  const payload = JSON.stringify({
    ...
    filialOrigem:      '57',  // filial GVR Home SP — confirmado no portal cliente.jamef.com.br
