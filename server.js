...
async function braspressCotacao(campos) {
  ...
  const valorMatch = html.match(/Valor Total Frete[\s\S]{0,20}?([\d]+\.[\d]{2})/i) ||
                     html.match(/([\d]+\.[\d]{2})/) ;
  ...
  const prazoMatch = html.match(/Data Entrega Prevista[\s\S]{0,10}?(\w+ \w+ \d+ [\d:]+ \w+ \d{4})/i);
  let prazo = null;
  if (prazoMatch) {
    try {
      const d = new Date(prazoMatch[1]);
      prazo = String(d.getDate()).padStart(2,'0') + '/' +
              String(d.getMonth()+1).padStart(2,'0') + '/' +
              d.getFullYear();
    } catch { prazo = prazoMatch[1]; }
  }
