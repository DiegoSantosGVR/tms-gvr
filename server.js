...
async function jamefCotacao(campos) {
  ...
  // Tenta /calculo-frete/v1/cotacoes — path confirmado pela documentação
  const cotacaoPath = JAMEF_API.basePath + '/cotacoes';
  const result = await httpsRequest({
    hostname: JAMEF_API.host,
    path:     cotacaoPath,
    method:   'POST',
...
async function braspressCotacao(campos) {
  ...
  // Guarda HTML completo para debug
  bpSession._lastHtml = html;
  console.log('[BRASPRESS HTML length]', html.length);
  console.log('[BRASPRESS HTML sample]', html.slice(0, 800));

  // Extrai protocolo
  const protMatch = html.match(/Protocolo[^:]*:?\s*<[^>]*>([\d]+)/i) ||
                    html.match(/protocolo[\s\S]*?([\d]{6,})/i);
  const protocolo = protMatch ? protMatch[1] : '—';

  // Extrai valor total
  const valorMatch = html.match(/Valor\s+Total[^R]*R\$\s*([\d.,]+)/i) ||
                     html.match(/R\$\s*([\d]+[.,][\d]{2})/);
  const valorStr   = valorMatch ? valorMatch[1] : null;
  const valorTotal = valorStr ? Number(valorStr.replace(/\./g,'').replace(',','.')) : null;

  // Extrai prazo
  const prazoMatch = html.match(/(\d{2}\/\d{2}\/\d{4})/);
  const prazo      = prazoMatch ? prazoMatch[1] : null;

  // Extrai dias úteis
  const diasMatch  = html.match(/(\d+)\s*[Dd]ia/);
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && pathname === '/health') {
    return json(res, 200, { status: 'ok', versao: '2.0.0', transportadoras: ['JAMEF ✅', 'BRASPRESS ✅'] });
  }
  ...
  try {
    ...
    if (req.method === 'GET' && pathname === '/api/braspress/debug') {
      const html = bpSession._lastHtml || 'Nenhuma resposta ainda';
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      res.end(html);
      return;
    }
